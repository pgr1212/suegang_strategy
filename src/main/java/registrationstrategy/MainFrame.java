...
public class MainFrame extends JFrame {
    ...
    private JPanel buildLeft() {
        ...
        btnReset.addActionListener(e -> { txtSearch.setText(""); refreshAllTable(courseManager.getCourseList()); });
        ...
        wishInfo.add(new JLabel("담은 과목:"));  wishInfo.add(lblWishCount);
        wishInfo.add(new JLabel("  |  총 학점:")); wishInfo.add(lblTotalCredit);
        ...
        wishBtns.add(btnAddWish); wishBtns.add(btnDelWish);
    ...
    private JPanel buildDetailPanel() {
        ...
        fldCode = UIHelper.createReadOnlyField(65, 22, 90); p.add(fldCode);
        addLabel(p, "과목명:", 165, 22, 45, 20);
        fldName = UIHelper.createReadOnlyField(210, 22, 250); p.add(fldName);

        // 행 2
        addLabel(p, "이수구분:", 8, 50, 55, 20);
        fldType = UIHelper.createReadOnlyField(65, 50, 50); p.add(fldType);
        addLabel(p, "학점:", 125, 50, 35, 20);
        fldCredit = UIHelper.createReadOnlyField(162, 50, 30); p.add(fldCredit);
        addLabel(p, "교수:", 200, 50, 35, 20);
        fldProf = UIHelper.createReadOnlyField(235, 50, 90); p.add(fldProf);
        addLabel(p, "강의시간:", 335, 50, 55, 20);
        fldTime = UIHelper.createReadOnlyField(392, 50, 65); p.add(fldTime);

        // 행 3
        addLabel(p, "경쟁률:", 8, 78, 45, 20);
        fldRate = UIHelper.createReadOnlyField(55, 78, 60); p.add(fldRate);
        addLabel(p, "위험도:", 125, 78, 45, 20);
        fldRisk = UIHelper.createReadOnlyField(172, 78, 60); p.add(fldRisk);
        addLabel(p, "위험점수:", 245, 78, 55, 20);
        fldScore = UIHelper.createReadOnlyField(302, 78, 50); p.add(fldScore);
        addLabel(p, "성공확률:", 360, 78, 55, 20);
        fldSuccessProb = UIHelper.createReadOnlyField(417, 78, 50); p.add(fldSuccessProb);
    ...
    private void addToWishList() {
        int row = tableAll.getSelectedRow();
        if (row < 0) { showMsg("과목을 먼저 선택하세요."); return; }
        String code = (String) modelAllCourses.getValueAt(row, 1);
    ...
    private void removeFromWishList() {
        ...
        if (row < 0) { showMsg("삭제할 과목을 선택하세요."); return; }
