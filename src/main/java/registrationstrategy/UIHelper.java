...
public class UIHelper {
    ...
    public static class RateColorRenderer extends DefaultTableCellRenderer {
        public RateColorRenderer() { setHorizontalAlignment(CENTER); }
    ...
    public static class RiskBgRenderer extends DefaultTableCellRenderer {
        public RiskBgRenderer() { setHorizontalAlignment(CENTER); }
        ...
        public Component getTableCellRendererComponent(
            ...
            if (!isSelected && value != null) {
                ...
                switch (s) {
                    case "매우 위험": c.setBackground(new Color(255, 220, 220)); c.setForeground(COLOR_DANGER); break;
                    case "주의":     c.setBackground(new Color(255, 240, 200)); c.setForeground(COLOR_WARNING); break;
                    case "보통":     c.setBackground(new Color(220, 245, 230)); c.setForeground(COLOR_NORMAL); break;
                    case "여유":     c.setBackground(new Color(210, 230, 255)); c.setForeground(COLOR_RELAXED); break;
    ...
    /** 경쟁률 바 렌더러 (JProgressBar 기반) */
    public static class RateBarRenderer extends DefaultTableCellRenderer {
        @Override
        public Component getTableCellRendererComponent(
                JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int col) {
            ...
            if (value != null) {
                try {
                    double rate = Double.parseDouble(value.toString().replace(":1", "").trim());
                    int val = (int) Math.min(rate * 100, 500);
                    bar.setValue(val);
                    bar.setString(String.format("%.2f:1", rate));
                    bar.setStringPainted(true);
                    if      (rate >= 3.0) bar.setForeground(new Color(200, 50, 50));
                    else if (rate >= 2.0) bar.setForeground(new Color(204, 120, 0));
                    else if (rate >= 1.0) bar.setForeground(new Color(0, 140, 70));
                    else                 bar.setForeground(new Color(0, 100, 200));
                    bar.setBackground(new Color(220, 220, 220));
                } catch (NumberFormatException ignored) {}
            }
            bar.setBorderPainted(false);
            bar.setFont(UIHelper.FONT_SMALL);
            return bar;
        }
    }
